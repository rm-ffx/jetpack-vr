	...
	public partial class PuppetMaster: MonoBehaviour {
		...
		protected virtual void SwitchStates() {
			...
			if (lastState == State.Alive) {
				if (state == State.Dead) StartCoroutine(AliveToDead());
				else if (state == State.Frozen) StartCoroutine(AliveToFrozen());
			}
		...
		private IEnumerator AliveToDead() {
			...
			/*
			// Gradually reducing muscle weight
			float mW = muscles[0].state.muscleWeightMlp;
			if (stateSettings.killDuration > 0f) {
				while (mW > stateSettings.deadMuscleWeight) {
					mW = Mathf.Max(mW - Time.deltaTime * (1f / stateSettings.killDuration), stateSettings.deadMuscleWeight);
		...
		private void SetAnimationEnabled(bool to) {
			if (targetAnimator != null) targetAnimator.enabled = to;
