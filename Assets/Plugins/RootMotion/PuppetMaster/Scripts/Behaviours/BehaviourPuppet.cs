...
namespace RootMotion.Dynamics {
	...
	public partial class BehaviourPuppet : BehaviourBase {
		...
		public override void KillStart() {
			...
			/*
			stateSwitchingEnabled = false;

			// Enable colliders, switch materials
			foreach (Muscle m in puppetMaster.muscles) {
				m.state.immunity = 0f;
				var props = GetProps(m.props.group);
		...
		protected override void OnFixedUpdate() {
			...
			if (state != State.Unpinned) {
				float knockOutDistanceSqr = Mathf.Sqrt(knockOutDistance);
				...
				foreach (Muscle m in puppetMaster.muscles) {
					...
					if (!puppetMaster.isBlending && m.state.pinWeightMlp < 0.5f && m.positionOffset.sqrMagnitude * m.props.pinWeight > props.knockOutDistance * knockOutDistanceSqr * stateMlp) {
		...
		protected override void OnLateUpdate() {
			forceActive = state != State.Puppet || puppetMaster.isKilling || !puppetMaster.isAlive;
		...
		protected override void OnReadBehaviour() {
			...
			if (!puppetMaster.isFrozen) {
				if (state == State.Unpinned && puppetMaster.isActive) {
			...
			if (getupAnimationBlendWeight > 0f) {
				...
				getUpPosition += y;
				puppetMaster.targetRoot.position = getUpPosition;
