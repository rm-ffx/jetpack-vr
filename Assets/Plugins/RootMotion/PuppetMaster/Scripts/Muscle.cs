...
namespace RootMotion.Dynamics {
	...
	public class Muscle {
		...
		public bool IsValid(bool log) {
			if (joint == null) {
				if (log) Debug.LogWarning("Muscle joint is null");
				return false;
			}
			
			if (target == null) {
				if (log) Debug.LogWarning("Muscle " + joint.name + "target is null, please remove the muscle from PuppetMaster or disable PuppetMaster before destroying a muscle's target.");
				return false;
			}
			
			if (props == null) {
				if (log) Debug.LogWarning("Muscle " + joint.name + "props is null");
			}
		...
		public virtual void Initiate(Muscle[] colleagues) {
			...
			if (joint.anchor != Vector3.zero) {
				Debug.LogWarning("PuppetMaster joint anchors need to be Vector3.zero. Joint axis on " + transform.name + " is " + joint.anchor, transform);
			}
		...
		public void UpdateColliders() {
			...
			if (_colliders.Length == 1) _colliders[0] = collider;
		...
		public void DisableColliders() {
			...
			for (int i = 0; i < colliders.Length; i++) {
				if (!colliders[i].enabled) disabledColliders.Add(i);
		...
		public void EnableColliders() {
			for (int i = 0; i < colliders.Length; i++) {
				...
				for (int d = 0; d < disabledColliders.Count; d++) {
					if (disabledColliders[d] == i) {
						enable = false;
						break;
					}
				}

				if (enable) colliders[i].enabled = true;
			}
		}
		...
		public void MoveToTarget() {
			...
			// Moving rigidbodies won't animate the pose
		...
		public void Read() {
			...
			if (readDeltaTime > 0f) targetVelocity = (target.position - targetAnimatedPosition) / readDeltaTime;

			if (props.mapPosition) targetLocalPosition = target.localPosition;

			targetAnimatedPosition = target.position;
			if (joint.connectedBody != null) targetAnimatedRotation = targetLocalRotation * localRotationConvert;
		}
		...
		public void UpdateAnchor(bool supportTranslationAnimation) {
			...
			Vector3 position = connectedBodyTarget.position;
			Quaternion rotation = connectedBodyTarget.rotation * toParentSpace;
			
			Vector3 anchorUnscaled = joint.connectedAnchor = InverseTransformPointUnscaled(position, rotation, target.position);
		...
		private Vector3 defaultPosition, defaultTargetLocalPosition;
		private Quaternion defaultLocalRotation, localRotationConvert, toParentSpace, toJointSpaceInverse, toJointSpaceDefault, 
		targetAnimatedRotation, defaultRotation, rotationRelativeToTarget, defaultTargetLocalRotation;
		...
		private List<int> disabledColliders = new List<int>(0);
		...
		private void Pin(float pinWeightMaster, float pinPow, float pinDistanceFalloff) {
			...
			Vector3 p = (positionOffset + offset) / Time.fixedDeltaTime;
		...
		private void MuscleRotation(float muscleWeightMaster, float muscleSpring, float muscleDamper) {
			...
			float d = props.muscleDamper * muscleDamper + state.muscleDamperAdd;
